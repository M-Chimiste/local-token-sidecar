...
void ST7701S::setup() {
  ...
  // Bounce buffer (internal RAM) absorbs PSRAM/bus contention while the LCD
  // scans. 10 lines is enough for a quiet bus, but a Wi-Fi DMA burst during the
  // metrics poll can briefly starve the refill and underrun one frame -> the
  // single poll-time flicker. 40 lines (~38KB internal) gives headroom to ride
  // out the burst. Pairs with CONFIG_LCD_RGB_ISR_IRAM_SAFE in the YAML so the
  // refill ISR stays responsive.
  config.bounce_buffer_size_px = this->width_ * 40;
  // Double-buffered: LVGL (full_refresh) renders a full frame, draw_bitmap
  // copies it into the back framebuffer, and the driver swaps at VSYNC -> the
  // panel never scans a half-written frame, so redraws (touch, changing token
  // totals) are tear-free. A single framebuffer made every redraw write into
  // the live, actively-scanned buffer, which flickered on each content change.
  // This can't cause static shimmer: with full_refresh, a flush only happens
  // when something is invalidated, so static content triggers no swap. The
  // earlier continuous shimmer with two framebuffers was actually bus/backlight
  // contention (2kHz backlight, Wi-Fi modem-sleep, tiny bounce buffer, non-IRAM
  // refill ISR), all fixed elsewhere; it was never the double buffer itself.
  config.num_fbs = 2;
